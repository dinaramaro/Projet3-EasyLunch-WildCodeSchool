      ...
      connection.query('SELECT booking_id FROM public_command WHERE booking_id = ?', id, (err2, results2) => {
        if (err2) {
          ...
          if (nb_users > nbIdBookingInCommand) {
            connection.query('SELECT restaurant_id FROM public_booking JOIN public_code ON public_booking.code = public_code.id WHERE name = ? AND free = 0 ORDER BY created_date DESC LIMIT 1', codeName, (err3, results3) => {
              if (err3) {
                res.sendStatus(500);
              } else {
                res.json(results3[0].restaurant_id);
              }
            });
          } else {
            res.sendStatus(401);
